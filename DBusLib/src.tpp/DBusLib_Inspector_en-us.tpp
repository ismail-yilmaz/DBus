...
connected peers. Like other DBusLib utility classes, it separates 
network requests from data retrieval. Client  code should first 
initiate a command (which returns true if the D`-Bus call succeeded), 
and then retrieve the parsed result using the corresponding Get 
method. Can be use both in blocking and non`-blocking mode.&]
[s3; &]
